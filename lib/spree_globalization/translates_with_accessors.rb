...
#         Spree::Config[:locale].split(",").map do |locale|
...
#       super
#
#       params_with_locale = Spree::Config[:locale].split(",").map do |locale|
#         params.map { |param| "#{param}_#{locale}".to_sym }
#       end
#
#       attr_accessible *(params_with_locale + params).flatten
#
#       globalize_accessors locales: Spree::Config[:locale].split(","), attributes: params.to_a if accessors
#     end
#   end
# end
#
